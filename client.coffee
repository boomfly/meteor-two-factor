...
getSelector = (user) ->
  if typeof user is 'string'
    if user.indexOf('@') isnt -1
      ...
      normalizedPhone = phone(user)?[0]
      if normalizedPhone
        return { 'phones.number': normalizedPhone }
      else
...
loginWithPassword = (options, cb) ->
  { user, password, emailCode, otpCode, captcha } = options
  # state = getState()
  selector = getSelector(user)
  # password = state.password

  Accounts.callLoginMethod {
    methodName: 'TwoFactor.login'
    methodArguments: [
      {
        user: selector
        password
  ...
  # console.log options
  # Meteor.call 'TwoFactor.login', {user: selector, password, emailCode, otpCode, captcha}, cb
